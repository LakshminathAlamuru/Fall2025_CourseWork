...
unsigned char process_builtin_commands(cmd_string *C)
{
  if (strcmp(C->cmd, "cd") == 0)
  {
    ...
    // if not directory mentions, change directory to home
    if(C->arg_count == 1)
    {
      char *home_dir = getenv("HOME");
      // check home directory is not null
      if(home_dir == NULL)
      {
        PRINT_ERROR;
        return (unsigned char) 1;
      }
      // check change directory status
      if(chdir(home_dir) == -1)
      {
  ...
  while (1)
  {
    ...
    else if (fc == 0)
    {
      ...
      if (C.redirection == 1)
      {
        ...
        int fd = open(C.ofile, O_CREAT | O_WRONLY | O_TRUNC, S_IRWXU);
        ...
        close(0); // It close the read from the stdin file if there is any
      }
       ...
       * if execvp() fails
       */
      int status = execvp(C.cmd, C.args);
      if(status == -1)
      {
