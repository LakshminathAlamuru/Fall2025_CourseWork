...
//preemptive priority with round robin time quantum for same priority jobs
void Scheduler::pp() {
    int low_index = 0;

    int low_prio = ready_queue->gethead()->priority;
    ...
    if(low_index >= 0)
    {
    	// apply the round robin logic if priority is same
         if(cpu->isidle() || (timer <= 0 || low_prio < cpu->getpcb()->priority))
         {
		timer = timeq;
		next_pcb_index = low_index;
		dispatcher->interrupt();
...
int getRandomValue(int min_val, int max_val)
{
...
//premptive random when priority is same
void Scheduler::pr(){
	// seed the random value instead of just pattern for rand()
	if(cpu->isidle() || timer <= 0)
	{
	    // max_val is considered as queue size
            if(ready_queue->size() > 0)
            {
    		  timer = timeq;
    		  int min_val = 0;
    		  int max_val = ready_queue->size()-1;
    		  next_pcb_index = getRandomValue(min_val, max_val);
              printf("pcb %d\n", next_pcb_index);
              dispatcher->interrupt();
            }
        }	
