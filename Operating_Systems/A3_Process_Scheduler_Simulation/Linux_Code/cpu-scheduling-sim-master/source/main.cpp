...
int main(int argc, char* argv[]) {
    ...
    if(atoi(argv[3]) == 3 && argc == 4){
        cout << "Need to provide time quantum when using Preemptive Priority algorithm" << endl;
        return EXIT_FAILURE;
    }
    
    if(atoi(argv[3]) == 4 && argc == 4){
        cout << "Need to provide time quantum when using Preemptive Random algorithm" << endl;
        return EXIT_FAILURE;
    }
    //variables to hold initial arguments
    int algorithm = atoi(argv[3]);
    int timeq = -1;
    //accept the time quantum for 2,3 and 4 algorithms as well
